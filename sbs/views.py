...
from .models import Items,t1,about,postjob,review,page,mesg,like,foll,cont,butn,post
...
def pages(request):
    ...
    return render(request,"pmain.html",context)
...
def page1(request,id):
    ...
    if request.method == 'POST':
        ...
        form1 = commen()
    return render(request, 'page.html',{'form' : form,'It' : It,'Pt' : Pt, 'form1':form1,'Ct':Ct})
...
def liked(request,id,paid):
    ...
    like.objects.create(user=user,paid=paid,pid=id,slug=user.username)
...
def create(request):
    ...
    return render(request, 'create.html',{'form':list1})
